...
int altitudePin = 10;
boolean prnt = false; // true to print to serial monitor
boolean plot = true; // should we plot the graph?
...
const int maxtime = 80; // length of run
...
void(* resetFunc) (void) = 0; // an abominable pointer to address 0 D:
...
void loop() {
  ...
  if(startMode == LOW) {
    startMode = digitalRead(startButtonPin);
  }
  else {
    simulator();
  }
